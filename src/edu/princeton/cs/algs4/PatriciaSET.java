 ...
 * <strong>~lg N</strong> for each <em>add</em>,
 * <em>contains</em>, or <em>delete</em> operation. Although this appears to put
 * PATRICIA on the same footing as binary trees, this time complexity represents
 * the number of single-bit test operations (under PATRICIA), and not full-key
 * comparisons (as required by binary trees). After the single-bit tests
 * conclude, PATRICIA requires just one full-key comparison to confirm the
 * existence (or absence) of the key (per <em>add</em>, <em>contains</em>, or
